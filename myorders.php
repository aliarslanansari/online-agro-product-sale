...
$un=$_SESSION['uname'];
$result=mysqli_query($conn,"select orders.quantity, orders.price, orders.address, 
						products.name, products.category,orders.id from orders,products 
where orders.uname='$un' and orders.prod_id = products.id");
$resultset=array();
...
<h2><b>MY ORDERS:</b></h2>
		...
		echo "<td> Name: ".$row[$j][3]." Quantity: ".$row[$j][0]."<br> Price(&#8377): ".$row[$j][1]."</p><a href='delorder.php?pid=".$row[$j][5]."'><button onclick=\"return  confirm('do you want to delete Y/N')\"> Cancel Order.</button></a></div></td>
		";
	}	
echo "</tr>";
} ?> </table>
